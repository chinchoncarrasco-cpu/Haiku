...
begin
  ...
  with candidatos as (
    select
      ...
      regexp_replace(
        translate(lower(coalesce(r.titular_nombre, '')), 'áéíóúüñ', 'aeiouun'),
        '[^a-z0-9]+', '', 'g'
    ...
    select c.*,
      case
        ...
        when v_nombre = '' then 50
        when c.nombre_normalizado = v_nombre then 100
        when c.nombre_normalizado like v_nombre || '%' or v_nombre like c.nombre_normalizado || '%' then 90
        when c.nombre_normalizado like '%' || v_nombre || '%' or v_nombre like '%' || c.nombre_normalizado || '%' then 80
        else 0
      end as score
    from candidatos c
  )
  select
    ...
    p.fecha_salida,
    p.total_alojamiento,
    p.pagado_alojamiento,
    p.saldo_alojamiento,
    coalesce((public.haiku_saldo_favor_unidad(p.reserva_id) ->> 'saldo_a_favor')::bigint, 0) as saldo_a_favor,
    p.score as coincidencia,
    coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'fecha_pago_chile', timezone('America/Santiago', x.fecha_pago)::date,
        'codaut', x.codigo_autorizacion,
        'folio', x.folio,
        'bovtar', x.bove,
        'glosa', x.referencia_externa
      ) order by x.fecha_pago desc, x.id desc)
      from (
        select pg.*
        from public.pagos pg
        where pg.reserva_id = p.reserva_id
          and pg.tipo_movimiento = 'pago'
          and pg.estado <> 'anulado'
        order by pg.fecha_pago desc, pg.id desc
        limit 10
      ) x
    ), '[]'::jsonb) as pagos_recientes
  from puntuados p
  where p.score > 0
  order by p.score desc,
           abs(p.fecha_ingreso - current_date),
...
begin
  ...
  select estado_reserva, titular_nombre
    into v_estado, v_titular
  from public.reservas
  where id = p_reserva_id
  for update;

  if not found then
    raise exception 'La reserva seleccionada no existe';
  end if;

  if v_estado = 'cancelada' then
    raise exception 'No se pueden registrar abonos en una reserva cancelada';
  end if;
  ...
  if jsonb_array_length(p_pagos) > 10 then
    raise exception 'Se admiten como máximo 10 abonos';
  end if;
  ...
  loop
    ...
    if v_medio = 'transferencia' and v_glosa is null then
    ...
    if v_medio in ('webpay_credito','webpay_debito') then
      ...
      if exists (
        ...
        where p.tipo_movimiento = 'pago'
          ...
          and p.medio_pago = v_medio
          and p.codigo_autorizacion = v_codaut
      ) then
        raise exception 'Ya existe un WebPay no anulado con el mismo CodAut y medio';
      end if;
    end if;
    ...
    if v_medio in ('tarjeta_credito','tarjeta_debito') then
      ...
      if exists (
        ...
        raise exception 'Ya existe un pago con tarjeta no anulado con el mismo Folio, BOVTAR y medio';
      end if;
    end if;
    ...
    v_pago := public.haiku_registrar_pago(
      ...
      p_observaciones => 'Abono confirmado desde asistente HAIKU sobre reserva existente',
    ...
    update public.pagos
       set verificado_por = auth.uid(),
           ...
           datos_origen = coalesce(datos_origen, '{}'::jsonb) || jsonb_build_object(
             'contexto', 'asistente_pago_reserva_existente',
    ...
    v_resultados := v_resultados || jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
      ...
      'codaut', case when v_medio in ('webpay_credito','webpay_debito') then v_codaut else null end,
      'glosa', case when v_medio = 'transferencia' then v_glosa else null end,
