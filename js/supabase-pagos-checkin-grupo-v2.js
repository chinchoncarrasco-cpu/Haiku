...
(() => {
    ...
    function htmlGrupo(g,f){
        ...
        <div class="pago-checkin-resumen-nuevo"><label class="pago-checkin-grupo"><span class="pago-checkin-label">Total grupo</span><div class="pago-checkin-input-monto"><span>$</span><input type="text" value="${Math.round(total).toLocaleString("es-CL")}" readonly></div></label><div class="pago-checkin-saldo-nuevo"><span>Saldo grupo</span><strong>${money(saldo)}</strong></div></div>
    ...
    async function aplicar(){
        ...
        try{
            ...
            for(const g of grupos){const ids=g.miembros.map(m=>m.reservaId),originales=ids.map(id=>lista.querySelector(`.pago-checkin-item[data-reserva-id="${CSS.escape(id)}"]`)).filter(Boolean);if(originales.length<2)continue;const f=await finanzas(ids[0]),card=document.createElement("div");card.className="pago-checkin-item haiku-checkin-grupo-v2";card.dataset.grupoId=g.grupoId;card.dataset.reservaId=ids[0];card.dataset.miembros=ids.join(",");card.innerHTML=htmlGrupo(g,f);originales[0].insertAdjacentElement("beforebegin",card);originales.forEach(el=>{el.hidden=true;el.dataset.haikuOcultoPorGrupo="1"});campos(card);}
